...
export default function RealtimeSubscription(
  ...
  useSignalEffect(() => {
    ...
    subscribeToRegistry(
      registryId,
      (payload) => {
        console.log("[RealtimeSubscription] Change received:", payload.eventType);
        status.value = `last-event: ${payload.eventType} @ ${new Date().toLocaleTimeString()}`;
      },
      accessToken,
    ).then(() => {
  ...
  return (
    <div class="fixed bottom-4 right-4 z-50 bg-card border border-white/10 rounded-custom px-3 py-2 text-xs text-slate-400 opacity-50 hover:opacity-100 transition-opacity">
      <span class="inline-block w-2 h-2 rounded-full mr-1.5 align-middle" style={`background-color: ${status.value === "subscribed" ? "#22c55e" : status.value === "connecting" ? "#eab308" : "#ef4444"}`} />
