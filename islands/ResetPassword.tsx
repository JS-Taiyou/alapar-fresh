...
export default function ResetPassword(props: ResetPasswordProps) {
  ...
  useEffect(() => {
    ...
    if (!accessToken || type !== "recovery") {
      error.value = "Enlace de recuperación inválido o expirado. Solicita uno nuevo.";
      return;
    }
    ...
    client.auth.setSession({
      ...
      if (sessionError) {
        error.value = "No se pudo establecer la sesión. El enlace puede haber expirado.";
        return;
      }
  ...
  if (error.value && !sessionReady.value) {
    return (
      <div class="space-y-4">
        <div class="text-sm text-red-400 bg-red-400/10 border border-red-400/20 rounded-custom px-4 py-3 text-center">
          {error.value}
        </div>
        <a
  ...
  if (success.value) {
    return (
      <div class="space-y-4">
        <div class="text-sm text-green-300 bg-green-400/10 border border-green-400/20 rounded-custom px-4 py-3 text-center">
