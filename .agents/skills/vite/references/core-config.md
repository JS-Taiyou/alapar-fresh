...
Vite auto-resolves `vite.config.ts` from project root. Supports ES modules
syntax regardless of `package.json` type.
...
`.env` files are loaded **after** config resolution. Use `loadEnv` to access
them in config:
...
Values must be JSON-serializable or single identifiers. Non-strings auto-wrapped
with `JSON.stringify`.
