...
export default define.page(function NewRegistry() {
  return (
    <>
      <Head>
        <title>A la par - Nuevo Registro</title>
      </Head>
      <main class="min-h-screen flex items-center justify-center p-6 bg-pattern">
        ...
        <div class="bg-surface border border-border-custom rounded-custom p-8 w-full max-w-md z-10">
          <div class="text-center mb-8">
            ...
            <p class="text-slate-400 text-sm mt-2">Crea un grupo para gestionar gastos compartidos</p>
          </div>
          <form action="/api/registries" method="POST" class="space-y-5">
            <div>
              <label class="block text-sm font-medium text-slate-300 mb-1.5" for="name">Nombre del Registro</label>
              <input
            ...
            <a href="/" class="block text-center text-sm text-slate-400 hover:text-white transition-colors">
              Cancelar
            </a>
          </form>
        </div>
