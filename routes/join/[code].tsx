          ...
          <div class="bg-surface border border-border-custom rounded-custom p-8 w-full max-w-md z-10 text-center">
            <div class="inline-flex items-center justify-center p-3 bg-red-500/20 rounded-custom mb-4">
              <svg class="h-8 w-8 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
              </svg>
            </div>
            <h1 class="text-2xl font-bold text-white mb-2">Invitación no encontrada</h1>
            <p class="text-slate-400 mb-6">El código <span class="font-mono text-white">{data.code}</span> no es válido o ha expirado.</p>
            <a href="/" class="inline-block px-6 py-3 bg-primary hover:bg-primary-light text-white font-semibold rounded-custom transition-all">
              Ir al inicio
            </a>
          </div>
        </main>
    ...
    <>
      ...
      <main class="min-h-screen flex items-center justify-center p-6 bg-pattern">
        ...
        <div class="bg-surface border border-border-custom rounded-custom p-8 w-full max-w-md z-10 text-center">
          ...
          <h1 class="text-2xl font-bold text-white mb-2">Unirse a {inv.registryName}</h1>
          <p class="text-slate-400 mb-6">Has sido invitado a un grupo de gastos compartidos.</p>
          ...
          {isInvalid
            ...
            : (
              <div class="space-y-3">
                <a href="/login" class="block w-full py-3 bg-primary hover:bg-primary-light text-white font-semibold rounded-custom transition-all shadow-lg">
                  Iniciar Sesión para Unirme
                </a>
                <a href="/signup" class="block w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold rounded-custom transition-all">
                  Crear Cuenta
                </a>
              </div>
            )}
