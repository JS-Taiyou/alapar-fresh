...
export const handlers = define.handlers({
  async GET(ctx) {
    ...
    if (!registryId) return { data: { grouped: {} as Record<number, Awaited<ReturnType<typeof getExercises>>>, years: [] as number[], exercises: [] as Awaited<ReturnType<typeof getExercises>> } };
        ...
        <div class="max-w-2xl mx-auto px-4 py-8">
          <header class="mb-8 flex items-center justify-between">
            <div class="flex items-center gap-4">
              <a class="p-2 hover:bg-slate-800 rounded-custom text-slate-400 hover:text-white transition-colors" href="/dashboard">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path d="M10 19l-7-7m0 0l7-7m-7 7h18" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
                </svg>
              </a>
              <div>
                <h1 class="text-2xl font-bold tracking-tight">Histórico de Cortes</h1>
                <p class="text-slate-400 text-sm">Consulta cierres de meses anteriores</p>
              </div>
            </div>
          ...
          <section class="space-y-4 mt-6">
            {data.years.map((year) => (
              <div key={year} class="space-y-2">
                <h2 class="text-xs font-semibold text-slate-500 uppercase tracking-widest px-2 py-1">{year}</h2>
                {data.grouped[year].map((ex) => (
                  <ExerciseCard
                    key={ex.id}
                    exercise={ex}
                    monthName={getMonthNameEs(ex.startDate).toUpperCase().substring(0, 3)}
                  />
                ))}
              </div>
            ))}
          ...
          {data.exercises.length === 0 && (
            <div class="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 class="text-lg font-medium text-slate-300">No hay cortes registrados</h3>
              <p class="text-slate-500 mt-2 max-w-xs mx-auto">Cuando realices un cierre de gastos aparecerán aquí.</p>
            </div>
          )}

          <footer class="mt-12 text-center border-t border-slate-800 pt-6">
            <p class="text-slate-500 text-xs italic">
              * Los cortes incluyen liquidaciones definitivas y facturas archivadas.
            </p>
          </footer>
        </div>
      </main>
