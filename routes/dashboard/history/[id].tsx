...
import { getExerciseById, getTransactionsByExercise, getUserById } from "../../../lib/store.ts";
...
export const handlers = define.handlers({
  async GET(ctx) {
    ...
    if (!exercise) return { data: { exercise: undefined, transactions: [] as EnrichedTransaction[] } };
...
export default define.page(function ExerciseDetail(ctx) {
  ...
  const currentRegistryUserId = currentUser
    ? ctx.state.registryUsers.find((u) => u.system_user_id === currentUser.id)?.id ?? ""
    : "";

  if (!exercise) {
    return (
      <>
        <Head><title>A la par - No encontrado</title></Head>
        <div class="flex-1 flex items-center justify-center">
          <div class="text-center">
            <h1 class="text-2xl font-bold text-white mb-2">Corte no encontrado</h1>
            <a href="/dashboard/history" class="text-primary hover:underline">Volver al histórico</a>
          </div>
        </div>
      </>
    );
  ...
  return (
    <>
      <Head>
        <title>A la par - Corte {monthNames[exercise.startDate.getMonth()]} {exercise.startDate.getFullYear()}</title>
      </Head>
      <main class="flex-1 overflow-y-auto custom-scrollbar">
        <div class="max-w-2xl mx-auto px-4 py-8">
          <header class="mb-8 flex items-center gap-4">
            <a class="p-2 hover:bg-slate-800 rounded-custom text-slate-400 hover:text-white transition-colors" href="/dashboard/history">
            ...
            <div>
              <h1 class="text-2xl font-bold tracking-tight">
                Corte {monthNames[exercise.startDate.getMonth()]} {exercise.startDate.getFullYear()}
              </h1>
